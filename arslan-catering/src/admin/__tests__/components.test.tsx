...
describe('DiffModal', () => {
  ...
  it('calls onClose on Escape', () => {
    ...
    render(<DiffModal tabKey="about" onClose={onClose} onRevertAll={vi.fn()} />)
    fireEvent.keyDown(window, { key: 'Escape' })
    expect(onClose).toHaveBeenCalledOnce()
  })
...
describe('TabEditor', () => {
  it('renders about tab', async () => {
    ...
    resetStore({ state: { about: aboutData }, originalState: { about: aboutData } })
  ...
  it('renders impressum tab (object with sections)', () => {
    const impressumData = { beschreibung: '', sections: [] }
    resetStore({ state: { impressum: impressumData }, originalState: { impressum: impressumData } })
    const tab = TABS.find(t => t.key === 'impressum')!
    const { container } = render(<TabEditor tab={tab} />)
    expect(container.firstChild).toBeTruthy()
  })

  it('renders startseite tab', () => {
    const startData = {
      heroTitel: '',
      heroTitelTr: '',
      heroUntertitel: '',
    ...
    resetStore({ state: { startseite: startData }, originalState: { startseite: startData } })
    const tab = TABS.find(t => t.key === 'startseite')!
    const { container } = render(<TabEditor tab={tab} />)
    expect(container.firstChild).toBeTruthy()
  })

  it('renders galerie tab (object type with sections)', () => {
    const galerieData = { titel: '', titelTr: '', beschreibung: '', beschreibungTr: '', bilder: [] }
    resetStore({ state: { galerie: galerieData }, originalState: { galerie: galerieData } })
