...
describe('persistDirtyState', () => {
  ...
  it('writes dirty tabs to localStorage after 1 second debounce', async () => {
    ...
    const state: Record<string, unknown> = { about: aboutData }
    ...
    expect(parsed.about.data).toEqual(aboutData)
  })
  ...
  it('debounces — only the last call within 1s takes effect', async () => {
    ...
    const state1: Record<string, unknown> = { about: { ...aboutBase, titel: 'first' } }
    const state2: Record<string, unknown> = { about: { ...aboutBase, titel: 'second' } }
    const original: Record<string, unknown> = { about: aboutBase }
    ...
    expect(parsed.about.data).toEqual({ ...aboutBase, titel: 'second' })
  })
})
