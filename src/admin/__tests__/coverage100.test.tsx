...
describe('AdminApp - publish-all with orphans', () => {
  it('shows OrphanModal when orphans are found during publishAll', async () => {
    ...
    const origData = {
      titel: 'A',
      beschreibung: '',
      bildUrl: '/images/about/removed.webp',
      werte: [],
    }
    resetStore({
      state: { about: aboutData },
      originalState: { about: origData },
      dataLoaded: true,
    })
    ...
    useAdminStore.setState({
      ...
      findOrphanImages: () => ['/images/about/removed.webp'],
    })
  ...
  it('handlePublishAllConfirmed calls publishAll when no orphans', async () => {
    const aboutData = { titel: 'A', beschreibung: '', bildUrl: '/images/about/a.webp', werte: [] }
    resetStore({
      state: { about: aboutData },
      originalState: {
        about: { titel: 'B', beschreibung: '', bildUrl: '/images/about/b.webp', werte: [] },
      },
      dataLoaded: true,
    })
...
describe('PreviewModal - uncovered branches', () => {
  ...
  it('replaces image URLs with pending upload data URIs', async () => {
    resetStore({
      ...
      pendingUploads: [{ ghPath: 'public/images/about/a.webp', base64: 'abc123', message: 'm' }],
    })
    const { container } = render(
      <MemoryRouter>
        <React.Suspense fallback={<div>loading</div>}>
          <PreviewModal tabKey="about" onClose={vi.fn()} />
        </React.Suspense>
      </MemoryRouter>,
    )
    await act(async () => {
  ...
  it('replaceUrls returns primitive values as-is', async () => {
    resetStore({
      ...
      originalState: { about: {} },
      pendingUploads: [{ ghPath: 'public/images/about/x.webp', base64: 'z', message: 'm' }],
    })
    const { container } = render(
      <MemoryRouter>
        <React.Suspense fallback={<div>loading</div>}>
...
describe('TabEditor - ObjectEditor and SectionEditor', () => {
  ...
  it('renders load-error banner with retry button', () => {
    ...
    resetStore({
      state: { about: { titel: 'A', beschreibung: '' } },
      originalState: { about: { titel: 'A', beschreibung: '' } },
      dataLoadErrors: ['about'],
    })
    const loadData = vi.fn()
    useAdminStore.setState({ loadData })
    const { container } = render(<TabEditor tab={aboutTab as TabConfig} />)
    expect(container.textContent).toContain('nicht geladen')
    const retryBtn = Array.from(container.querySelectorAll('button')).find(b =>
      b.textContent?.includes('Erneut versuchen'),
    )
...
describe('DiffModal - structural revert button (line 178)', () => {
  it('renders structural revert button for added items', () => {
    resetStore({
      ...
      originalState: { galerie: { titel: '', beschreibung: '', bilder: [] } },
    })
    const { container } = render(
  ...
  it('renders structural group for removed items', () => {
    resetStore({
      state: { galerie: { titel: '', beschreibung: '', bilder: [] } },
      originalState: {
        galerie: {
          titel: '',
          beschreibung: '',
          bilder: [{ url: '/images/galerie/a.webp', beschreibung: 'Old' }],
        },
      },
    })
    const { container } = render(
  ...
  it('renders confirm revert-all flow', () => {
    resetStore({
      ...
      originalState: { about: { titel: 'Original', beschreibung: '' } },
    })
    const onRevertAll = vi.fn()
    const { container } = render(
      <DiffModal tabKey="about" onClose={vi.fn()} onRevertAll={onRevertAll} />,
    )
  ...
  it('revert-all cancellation works', () => {
    ...
    const { container } = render(
      <DiffModal tabKey="about" onClose={vi.fn()} onRevertAll={vi.fn()} />,
    )
    const revertAllBtn = Array.from(container.querySelectorAll('button')).find(b =>
      b.textContent?.includes('Alle zurücksetzen'),
    )
...
describe('GlobalDiffModal - revert-all flow', () => {
  it('renders changes and can revert all', async () => {
    ...
    const aboutState = useAdminStore.getState().state.about as { titel: string }
    expect(aboutState?.titel).toBe('Original')
  })
  ...
  it('no changes shows empty message', () => {
    resetStore({
      state: { about: { titel: 'Same', beschreibung: '' } },
      originalState: { about: { titel: 'Same', beschreibung: '' } },
    })
    const { container } = render(<GlobalDiffModal onClose={vi.fn()} />)
...
describe('collectImagePaths - object tab with sections', () => {
  it('collects image URLs from object tab with sections', () => {
    const tab: TabConfig = {
      ...
      type: 'object',
      topFields: [{ key: 'heroBild', label: 'Hero-Bild', type: 'image' }],
      sections: [
        {
          key: 'saeale',
          label: 'Säle',
          fields: [
            { key: 'vorschau', label: 'Vorschau', type: 'image' },
            { key: 'bilder', label: 'Bilder', type: 'imagelist' },
          ],
        },
      ],
    }
    const data = {
      heroBild: '/images/hero/main.webp',
      saeale: [
        {
          vorschau: '/images/venues/saal1.webp',
    ...
    expect(paths.has('/images/hero/main.webp')).toBe(true)
    expect(paths.has('/images/venues/saal1.webp')).toBe(true)
    expect(paths.has('/images/venues/a.webp')).toBe(true)
    expect(paths.has('/images/venues/b.webp')).toBe(true)
    ...
    resetStore({
      ...
      originalState: {
        about: {
          ...
          beschreibung: '',
          beschreibungTr: '',
          bildUrl: '/images/about/old.webp',
          werte: [],
        },
      },
    })
    const orphans = useAdminStore.getState().findOrphanImagesForTab('about')
    expect(orphans).toContain('/images/about/old.webp')
  })
