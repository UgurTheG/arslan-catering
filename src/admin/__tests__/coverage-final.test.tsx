...
describe('diff.ts — additional branches', () => {
  ...
  it('applyRevert with unknown kind returns current', async () => {
    ...
    const tab = TABS.find(t => t.key === 'about')! as TabConfig
    const entry = {
      id: 'test',
      path: ['titel'],
    ...
    const result = applyRevert(
      tab,
      { titel: 'Orig', titelTr: '', beschreibung: '', beschreibungTr: '', bildUrl: '', werte: [] },
      { titel: 'Curr', titelTr: '', beschreibung: '', beschreibungTr: '', bildUrl: '', werte: [] },
      entry,
    )
