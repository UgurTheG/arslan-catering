...
describe('TabEditor - SectionEditor isSingleObject', () => {
  ...
  it('renders galerie tab with bilder section', () => {
    const galerieData = {
      titel: 'Galerie',
      titelTr: '',
      beschreibung: 'Fotos',
    ...
    const { container } = render(<TabEditor tab={tab} />)
    expect(container.textContent).toContain('Bilder')
  })

  it('renders venues tab with saeale section', () => {
    const venuesData = {
      titel: 'Säle',
      titelTr: '',
      beschreibung: '',
    ...
    resetStore({
      state: { venues: venuesData },
      originalState: { venues: venuesData },
    })
    const tab = TABS.find(t => t.key === 'venues')!
    const { container } = render(<TabEditor tab={tab} />)
    expect(container.textContent).toContain('Säle')
  })

  it('renders videos tab with videos section', () => {
    const videosData = {
      titel: 'Videos',
      titelTr: '',
      beschreibung: '',
      beschreibungTr: '',
      videos: [{ titel: 'Video 1', titelTr: '', typ: 'youtube', url: '', datum: '', vorschau: '' }],
    }
    resetStore({
      state: { videos: videosData },
      originalState: { videos: videosData },
    })
    const tab = TABS.find(t => t.key === 'videos')!
    const { container } = render(<TabEditor tab={tab} />)
    expect(container.firstChild).toBeTruthy()
  })

  it('renders datenschutz tab (object with topFields)', () => {
    const datenschutzData = { beschreibung: 'DS-Info', sections: [] }
    resetStore({
      state: { datenschutz: datenschutzData },
      originalState: { datenschutz: datenschutzData },
    })
    const tab = TABS.find(t => t.key === 'datenschutz')!
    const { container } = render(<TabEditor tab={tab} />)
    expect(container.firstChild).toBeTruthy()
  })
...
describe('DiffModal - confirm revert all and cancel', () => {
  it('cancels revert all confirmation', () => {
    resetStore({
      state: {
        galerie: { titel: 'New', titelTr: '', beschreibung: '', beschreibungTr: '', bilder: [] },
      },
      originalState: {
        galerie: { titel: 'Old', titelTr: '', beschreibung: '', beschreibungTr: '', bilder: [] },
      },
    })
