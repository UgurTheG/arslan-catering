...
describe('uiSlice — darkMode IIFE branches via module reset', () => {
  ...
  it('initializes darkMode=false when localStorage has "false"', async () => {
    localStorage.setItem('arslan-darkmode', 'false')
    vi.resetModules()
    const { useAdminStore: freshStore } = await import('../../admin/store/index')
...
describe('diff.ts — captionsKey companion path (lines 83-86)', () => {
  it('adds companion path when both imagelist and captions change', async () => {
    ...
    const entries = diffTab(customTab, original, current)
...
describe('DiffModal — keydown handler (line 26)', () => {
  it('fires Escape via window.dispatchEvent', () => {
    resetStore({
      ...
      originalState: {
        galerie: { titel: 'B', titelTr: '', beschreibung: '', beschreibungTr: '', bilder: [] },
      },
    })
    const onClose = vi.fn()
    render(<DiffModal tabKey="galerie" onClose={onClose} onRevertAll={vi.fn()} />)
    act(() => {
...
describe('GlobalDiffModal — removed and moved item kinds', () => {
  it('shows Entfernt badge for removed items (line 228)', () => {
    ...
    resetStore({
      state: {
        galerie: { titel: '', titelTr: '', beschreibung: '', beschreibungTr: '', bilder: [] },
      },
      originalState: {
        galerie: {
          titel: '',
          ...
          beschreibung: '',
          beschreibungTr: '',
          bilder: [{ url: '/images/galerie/a.webp', beschreibung: '', beschreibungTr: '' }],
        },
      },
    })
    ...
    resetStore({
      state: {
        galerie: {
          ...
          beschreibungTr: '',
          bilder: [
            { url: '/images/galerie/b.webp', beschreibung: 'B', beschreibungTr: '' },
            { url: '/images/galerie/a.webp', beschreibung: 'A', beschreibungTr: '' },
          ],
        },
      },
      originalState: {
    ...
    resetStore({
      ...
      originalState: {
        galerie: {
          ...
          beschreibungTr: '',
          bilder: [
            { url: '/images/galerie/a.webp', beschreibung: '', beschreibungTr: '' },
            { url: '/images/galerie/b.webp', beschreibung: '', beschreibungTr: '' },
          ],
        },
      },
    })
...
describe('PublishConfirmModal — removed and moved item kinds', () => {
  it('shows Entfernt badge for a removed item (lines 173-178)', () => {
    ...
    const { container } = render(
      <PublishConfirmModal tabKey="galerie" onConfirm={vi.fn()} onCancel={vi.fn()} />,
    )
    expect(container.textContent).toContain('Entfernt')
  })
...
describe('useUndoRedoShortcuts — isTextInput blocks undo/redo (lines 21, 27)', () => {
  it('Ctrl+Z on a focused INPUT does not trigger undo (line 21)', () => {
    ...
    resetStore({
      ...
      originalState: { about: aboutState },
      undoStacks: { about: [JSON.stringify({ ...aboutState, titel: 'B' })] },
    })
    const aboutTab = TABS.find(t => t.key === 'about')!
    const { container } = render(<TabEditor tab={aboutTab as TabConfig} />)
